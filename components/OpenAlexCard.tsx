...
export default function OpenAlexCard({ work }: OpenAlexCardProps) {
  const getCitationColor = (count: number) => {
    if (count >= 100) return 'bg-accent-green/10 text-accent-green border-accent-green/30';
    if (count >= 50) return 'bg-accent-blue/10 text-accent-blue border-accent-blue/30';
    if (count >= 10) return 'bg-amber-500/10 text-amber-400 border-amber-500/30';
    return 'bg-dark-elevated text-dark-text-muted border-dark-border';
  };
